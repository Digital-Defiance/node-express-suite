...
describe('TransactionManager', () => {
  ...
  describe('execute with transactions enabled', () => {
    ...
    it('should execute callback within transaction', async () => {
      ...
      mockSession.withTransaction.mockImplementation(async (cb) => await cb(mockSession));
    ...
    it('should end session even if transaction fails', async () => {
      ...
      await expect(manager.execute(async () => 'result')).rejects.toThrow('Transaction failed');
      expect(mockSession.endSession).toHaveBeenCalled();
    });
  ...
  describe('edge cases', () => {
    ...
    it('should handle session start failure', async () => {
      ...
      await expect(manager.execute(async () => 'result')).rejects.toThrow('Session start failed');
    });
    ...
    it('should handle endSession failure gracefully', async () => {
      ...
      await expect(manager.execute(async () => 'result')).rejects.toThrow('End session failed');
    });
  });
