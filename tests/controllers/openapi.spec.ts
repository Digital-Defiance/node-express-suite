/**
 * @fileoverview Unit and integration tests for OpenApiController
 */
...
import { OpenApiController } from '../../src/controllers/openapi';
import { IApplication } from '../../src/interfaces/application';
import { OpenAPIBuilderConfig } from '../../src/openapi/builder';
...
const createTestConfig = (): OpenAPIBuilderConfig => ({
  title: 'BrightChain API',
  version: '0.12.0',
  description: 'BrightChain API documentation',
});

describe('OpenApiController', () => {
  let app: Express;
  let mockApplication: IApplication<Buffer>;
  let controller: OpenApiController<Buffer>;
  ...
  describe('constructor', () => {
    it('should create controller instance', () => {
      controller = new OpenApiController(mockApplication, createTestConfig());
    ...
    it('should register itself with ControllerRegistry', () => {
      controller = new OpenApiController(mockApplication, createTestConfig());
      expect(ControllerRegistry.has('OpenApiController')).toBe(true);
    });
  });
  ...
  describe('GET /api/openapi', () => {
    beforeEach(() => {
      controller = new OpenApiController(mockApplication, createTestConfig());
      app.use('/api/openapi', controller.router);
    });
    ...
    it('should return correct API info', async () => {
      ...
      expect(response.body.info.title).toBe('Express Suite API');
      expect(response.body.info.version).toBe('0.12.0');
      expect(response.body.info.description).toContain('Express Suite');
    });
  ...
  describe('integration with other controllers', () => {
    beforeEach(() => {
      // Register some mock controllers before creating OpenApiController
