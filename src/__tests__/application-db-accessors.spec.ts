...
describe('Application db and getModel accessors', () => {
  ...
  describe('db getter', () => {
    it('should throw when no database plugin is registered', () => {
      const env = makeEnv();
      const app = createTestApplication(env);

      expect(() => app.db).toThrow('No MongoDatabasePlugin registered');
    });
  ...
  describe('getModel', () => {
    it('should throw when no database plugin is registered', () => {
      ...
      expect(() => app.getModel('User')).toThrow(
        'No MongoDatabasePlugin registered',
      );
    });
