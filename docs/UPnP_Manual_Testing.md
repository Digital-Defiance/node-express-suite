...
This guide walks through testing UPnP port mapping with a real router using `@digitaldefiance/node-express-suite`. UPnP allows the application to automatically forward ports on your router so it's reachable from the internet without manual configuration.
...
3. Your Express application running on a machine **connected to the router's LAN** (wired or Wi-Fi).
...
Set the UPnP environment variables for your application:
...
Start your Express application with UPnP enabled. Watch the console output for UPnP log messages. A successful initialization looks like:
...
|---------------|-----------------|---------------|----------|------------------------|
| 3000          | 192.168.1.x     | 3000          | TCP      | Express App HTTP       |

If you configured a separate WebSocket port, you'll see a second entry for "Express App WebSocket" (or your custom `descriptionPrefix`).
...
Replace `<your-external-ip>` with the IP logged during initialization. You should get a response from the Express server (the exact response depends on your routes — a connection refusal or timeout means the mapping isn't working).
...
Go back to your router's admin page and confirm the UPnP port mapping entry is **gone**. Also re-run the external port check from step 4 — the port should now be **closed**.
...
- A previous application instance didn't shut down cleanly and left a stale mapping. Restart the router's UPnP service or wait for the old mapping's TTL to expire.
...
6. Set `UPNP_ENABLED=false` in your environment to skip UPnP initialization.
