...
`@digitaldefiance/node-express-suite` supports automatic port forwarding via UPnP (Universal Plug and Play) and NAT-PMP. When enabled, the application asks your router to forward the configured ports so it's reachable from the internet — no manual router configuration required.

---

## Quick Start

1. Set the UPnP environment variables in your application's environment:

   ```dotenv
   UPNP_ENABLED=true
   ```

2. Register the UPnP plugin (or use `UpnpManager` directly):
   ...
   if (config.enabled) {
     const manager = new UpnpManager({ config, descriptionPrefix: 'My App' });
     await manager.initialize();
   }
   ```

3. Start the server. On startup you'll see log output confirming the mapping:
...
| `UPNP_REFRESH_INTERVAL` | number | `1800000` | Must be < `UPNP_TTL × 1000` | How often (in **milliseconds**) the application renews its port mappings. Default is 30 minutes. |
...
When using `UpnpPlugin`, you can override environment-based settings via the options object:
...
Fields not specified in the override will fall back to environment variables, then to defaults.
...
3. Create a rule:
   ...
   - **Protocol:** TCP
   - **Description:** Your App HTTP
4. If you use a separate WebSocket port, create a second rule for that port.
5. Save and apply.
6. Set `UPNP_ENABLED=false` in your environment so the application skips UPnP initialization.
...
Another device or a previous application instance already holds a mapping for the same port.
...
- **TTL limits.** Mappings are created with a finite TTL (default 1 hour, minimum 60 seconds). If the application stops refreshing, the mapping expires automatically on the router.
- **Cleanup on shutdown.** The application removes all port mappings during graceful shutdown (`SIGTERM` / `SIGINT`). If the process is killed forcefully (`SIGKILL`) or the network is unavailable, the mapping expires after the TTL.
- **Descriptive mapping names.** Mappings are labelled with a configurable prefix (e.g. `"My App HTTP"`) so they're easy to identify in your router's UPnP table.
