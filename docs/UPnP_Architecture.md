...
This document covers the UPnP port mapping subsystem in `@digitaldefiance/node-express-suite`: API reference, integration patterns, runtime flows, and security considerations.
...
│                                                                 │
│  PluginManager  ──►  UpnpPlugin.init(app)                       │
│                         │                                       │
│                         ▼                                       │
│                    UpnpManager (managed by plugin)               │
│                                                                 │
└─────────────────────────────┬───────────────────────────────────┘
                              │ uses
┌─────────────────────────────▼───────────────────────────────────┐
...
All UPnP types, services, configuration, manager, and plugin are exported from a single package: `@digitaldefiance/node-express-suite`. The plugin system provides automatic lifecycle management, while standalone classes can be used independently.
...
Server-level orchestrator. Manages the full lifecycle of UPnP mappings within an Express application.

#### Constructor

```typescript
new UpnpManager(configOrOptions: IUpnpConfig | UpnpConfig | UpnpManagerOptions)
```

Accepts either a config directly (backward compatible) or an options object with an optional `descriptionPrefix` (defaults to `'Express App'`).

```typescript
interface UpnpManagerOptions {
  config: IUpnpConfig | UpnpConfig;
  descriptionPrefix?: string; // defaults to 'Express App'
}
```
...
Plugin adapter implementing `IApplicationPlugin` that wires `UpnpManager` into the express-suite plugin lifecycle.
...
| Method | Signature | Description |
|--------|-----------|-------------|
| `init` | `(app: IApplication) => Promise<void>` | Load config, create and initialize UpnpManager |
| `stop` | `() => Promise<void>` | Shut down the UpnpManager |
| `getManager` | `() => UpnpManager \| null` | Access the underlying manager (e.g. for endpoint queries) |
...
```typescript
import { UpnpService } from '@digitaldefiance/node-express-suite';
...
```typescript
import { UpnpConfig, UpnpManager } from '@digitaldefiance/node-express-suite';

// During app startup, after HTTP server is listening:
...
```typescript
import { PluginManager, UpnpPlugin } from '@digitaldefiance/node-express-suite';

const pluginManager = new PluginManager();
pluginManager.register(new UpnpPlugin({
  descriptionPrefix: 'My App',
}));

// Plugins are initialized/stopped automatically with the application lifecycle
```

### Peer Advertisement

```typescript
// After UPnP is initialized, get external endpoints for peer discovery:
...
App.start()
    ...
    ├─ if config.enabled:
    │   │
    │   ├─ new UpnpManager({ config, descriptionPrefix })
...
- Each mapping carries a descriptive label (e.g. `"Express App HTTP"`, `"Express App WebSocket"`) so router admin UIs show clear ownership. The label prefix is configurable via `descriptionPrefix`.
...
Located at `src/services/__tests__/upnp.spec.ts`:
...
Located at `src/services/__tests__/upnp.property.spec.ts`:
- Config merge with defaults, invalid port validation, mapping round trips
- Retry logic, IP caching, closed service rejection

### Manager Tests

Located at `src/services/__tests__/upnp-manager.spec.ts`:
- Mock `UpnpService` to test `UpnpManager` lifecycle
- Startup, shutdown, refresh timer, signal handling, configurable description prefix

### Config Tests

Located at `src/services/__tests__/upnp-config.spec.ts`:
- Environment variable parsing, default fallback, validation error cases

### Plugin Tests

Located at `src/plugins/__tests__/upnp.spec.ts`:
- Plugin init with enabled/disabled config, stop delegation, manager accessor

### Running Tests

```bash
# All UPnP tests in express-suite
NX_TUI=false npx nx test digitaldefiance-node-express-suite --outputStyle=stream
```
